  ...
  function INITCONFIG() {
    ...
    const actionMap = {
      ...
      "layPhanLoaiTiktok": layPhanLoaiTiktok,
      ...
      "layPhanLoaiLazada": layPhanLoaiLazada,
      ...
      "tinhGiaTruocKM": tinhGiaTruocKM,
      "gopGiaBan": gopGiaBan,
      "tachGiaBan": tachGiaBan,
    };
    ...
    function createLayout(mainContent) {
      ...
      mainContent.append($(`
                ...
                <option data-func="layPhanLoaiTiktok" data-layout="layPhanLoaiTiktokLayout">Lấy Phân Loại</option>
							...
							<optgroup label="Lazada">
								...
								<option data-func="layPhanLoaiLazada" data-layout="layPhanLoaiLazadaLayout">Lấy Phân Loại</option>
							...
							<optgroup label="Khác">
								...
								<option data-func="tinhGiaTruocKM" data-layout="tinhGiaTruocKMLayout">Tính Giá Trước KM</option>
								<option data-func="gopGiaBan" data-layout="gopGiaBanLayout">Gộp Giá Bán</option>
								<option data-func="tachGiaBan" data-layout="tachGiaBanLayout">Tách Giá Bán</option>
      ...
      let isResizing = false,
        containers, startX, startY, startWidth, startHeight;
      ...
      switch (layoutName) {
        ...
        case "aiChatLayout":
          ...
          $(".layout-tab .typing-content input").css({
            "flex-grow": "1",
            /* Cho phép input chiếm phần lớn chiều rộng */
            "height": "40px",
            "padding": "10px",
    ...
    function createLayoutTab(layoutName) {
      ...
      $(".layout-tab").remove();
      switch (layoutName) {
        case "tachGiaBanLayout":
          content.append($(`
						<div class="layout-tab">
							<textarea id="data" placeholder="Mỗi dòng là một giá cần tách giá đầu và đuôi"></textarea>
						</div>
				`))
          break;
        case "gopGiaBanLayout":
          content.append($(`
					<div class="layout-tab">
						<textarea id="data" placeholder="Mỗi dòng là một giá, trong đó\n - Giá trước KM\n - Giá sau KM"></textarea>
					</div>
				`))
          break;
        case "tinhGiaTruocKMLayout":
          content.append($(`
					<div class="layout-tab">
						<input type="number" id="percent" placeholder="Nhập phần trăm muốn tăng, mặc định là 30%" />
						<textarea id="data" placeholder="Mỗi dòng là một giá đuôi"></textarea>
					</div>
				`))
          break;
        case "suaTenSKUShopeeLayout":
          content.append($(`
					<div class="layout-tab">
						<textarea id="data" placeholder="Mỗi phân loại là một hàng, trong đó:\n - SKU\n - Tên Phân Loại\nSố lượng ký tự sẽ được đếm tự động không cần quan tâm"></textarea>
					</div>
				`));
          setEventSuaTenSKUShopee();
          break;
    ...
    async function suaHinhSKUShopee() {
      boxLogging("SỬA HÌNH ẢNH SHOPEE (Tìm kiếm SKU Dự Phòng & Sửa lỗi cú pháp Promise)", [], ["purple", "bold"]);
      ...
      // 1. Kiểm tra và Tính toán Tỷ lệ SKU trên mỗi nhóm
      if (boxLeft.length === 0) {
      ...
      // --- Hàm TÌM KIẾM SKU CÓ ẢNH TRONG NHÓM (Giữ nguyên) ---
      function findMatchingSkuAndFile(groupStartIndex, groupSize, totalSkuCount) {
          for (let i = 0; i < groupSize; i++) {
              const currentSkuIndex = groupStartIndex + i;
              
              if (currentSkuIndex >= totalSkuCount) continue; 
              
              var currentSkuBox = box.eq(currentSkuIndex);
              var skuTextarea = currentSkuBox.find(".table-cell").eq(2).find("textarea");
              var sku = skuTextarea.val().trim().toUpperCase();

              if (inputMap[sku] && inputMap[sku].get(0).files.length > 0) {
                  return {
                      sku: sku,
                      fileInputEl: inputMap[sku].get(0)
                  };
              }
          }
          return { sku: null, fileInputEl: null };
      }

      // 2. Tạo mảng Promises (Sửa lỗi cú pháp)
      const promises = [];

      for (let index = 0; index < boxLeft.length; index++) {
          
          // --- Đẩy Promise được tạo bởi hàm async vào mảng ---
          promises.push((async (index) => { 
              const skuRepresentativeIndex = index * SKU_PER_GROUP;
              const { sku: matchingSku, fileInputEl } = findMatchingSkuAndFile(skuRepresentativeIndex, SKU_PER_GROUP, box.length);
              ...
              if (matchingSku) {
                  ...
                  if (boxLeft.eq(index).find(".table-cell img.shopee-image-manager__image").length > 0) {
                      ...
                      boxLogging(`Đã xóa ảnh cũ cho nhóm SKU (match: [copy]${matchingSku}[/copy])`, [`${matchingSku}`], ["orange"]);

                      boxLeft.eq(index).css({ "background": "orange", "color": "#000" });
                      await delay(500); 
                  }

                  // 2. Gán và Upload ảnh mới
                  ...
                  await new Promise(resolve => {
                      setTimeout(() => {
                          ...
                          boxLogging(`Đã gán ảnh mới cho nhóm (SKU match: [copy]${matchingSku}[/copy])`, [`${matchingSku}`], ["green"])
                          boxLeft.eq(index).css({ "background": "lightgreen", "color": "#000" });
                          resolve();
                      }, 100);
                  });

                  // 3. Đánh dấu tất cả các SKU phụ thuộc nhóm này
                  for (let i = 0; i < SKU_PER_GROUP; i++) {
                      ...
                      if (currentSkuIndex < box.length) {
                          ...
                          if (subSku !== matchingSku) {
                              boxLogging(`SKU phụ [copy]${subSku}[/copy] dùng chung ảnh nhóm`, [`${subSku}`], ["green"]);
                          }
                      }
                  }

              } else {
                  // Trường hợp KHÔNG tìm thấy ảnh cho BẤT KỲ SKU nào trong nhóm
                  
                  var currentSkuBox = box.eq(skuRepresentativeIndex);
                  var skuRepresentative = currentSkuBox.find(".table-cell").eq(2).find("textarea").val().trim().toUpperCase();
                  
                  boxLogging(`NHÓM LỖI: Không tìm thấy ảnh trong inputMap cho BẤT KỲ SKU nào (bắt đầu bằng [copy]${skuRepresentative}[/copy])`, [`${skuRepresentative}`], ["crimson"]);
          ...
          })(index)); // Gọi IIFE và đẩy kết quả (một Promise) vào mảng
      }
      ...
      $.each(data.variants, (index, valueData) => {
        ...
        $.each(box, async (index, valueBox) => {
          ...
          if (sku.includes(valueData.sku.toUpperCase())) {
            ...
            var file = await base64ToFile(img, `${valueData.sku}.jpg`);
            ("input[type=file]")[0];
    ...
    async function mergeExcelFile() {
      ...
      const mergedAuxiliaryHeaderRows = Array.from({
        length: descriptionRowsCount
      }, () => []);
    ...
    async function suaTonSKUShopee(...data) { // Đảm bảo hàm này là ASYNC
      ...
      try {
        ...
        mainTableWrapperContent = await awaitForElement($("body"), ".variation-model-table-main .eds-scrollbar.middle-scroll-container .eds-scrollbar__content .variation-model-table-body", {
          timeout: 15000
        });
      ...
      for (let indexBox = 0; indexBox < box.length; indexBox++) {
        ...
        if (foundSkuIndex !== -1) {
          ...
          try {
            warehouseEditModal = await awaitForElement($("body"), ".multi-warehouse-stock-edit", {
              timeout: 7000
            });
          ...
          try {
            var confirmButton = await awaitForElement($(warehouseEditModal), ".stock-edit-fixed .stock-edit-fixed-right button.eds-button--primary", {
              timeout: 3000
            });
            ...
            try {
              var closeButton = await awaitForElement($(warehouseEditModal), ".shopee-modal__close-btn", {
                timeout: 1000
              });
    ...
    async function themHinhTheoSKUTiktok() {
      ...
      for (let i = 0; i < variantImageContainers.length; i++) {
        ...
        if (foundMapping) {
          ...
          $(nameInputForVariant).get(0).scrollIntoView({
            behavior: 'smooth',
            block: 'center'
          });;
    ...
    async function themHinhTheoSKULazada() {
			boxLogging("THÊM HÌNH ẢNH THEO SKU LAZADA (Sử dụng logic ROWSPAN để gán nhóm)", [], ["darkblue"]);
			...
			if (tableRows.length === 0) {
				boxLogging("Không tìm thấy bảng SKU biến thể.", [], ["red"]);
				boxToast("Không tìm thấy bảng SKU!", "error");
				return;
			}
			boxLogging(`Đang thu thập thông tin SKU từ ${tableRows.length} hàng biến thể trên sàn...`, [], ["blue"]);

			// **Bước 1: SỬA LỖI LOGIC DỰA TRÊN ROWSPAN**
			let activeGroupName = null;
			let rowsRemainingInGroup = 0; // Đếm số hàng còn lại thuộc về activeGroupName
			...
			for (let i = 0; i < tableRows.length; i++) {
				...
				// Lấy cột đầu tiên của dòng hiện tại
				var firstCell = row.find("td").eq(0);
				var rowspanAttr = firstCell.attr('rowspan');

				if (rowspanAttr) {
					// --- HÀNG ĐẠI DIỆN ---
					// Phát hiện ra Tên Cấp 1 mới vì có thuộc tính rowspan
					rowsRemainingInGroup = parseInt(rowspanAttr, 10);

					var nameBoxC1 = firstCell.find(".next-btn");
					groupName = nameBoxC1.length > 0 ? nameBoxC1.text().trim() : "";

					if (groupName.length === 0) {
						// Thử lấy từ văn bản thuần túy
						groupName = firstCell.text().trim().split('\n')[0].trim();
					}

					activeGroupName = groupName; // Cập nhật Tên nhóm đang hoạt động
					skuColumnIndex = 5; // Hàng đại diện: SKU ở cột 5

				} else if (rowsRemainingInGroup > 0) {
					// --- HÀNG PHỤ (Vẫn thuộc nhóm đang hoạt động) ---
					groupName = activeGroupName;
					skuColumnIndex = 4; // Hàng phụ: SKU ở cột 4 (do cột 1 đã bị rowspan)
				} else {
					// Đây là trường hợp dòng lỗi/dòng trống không thuộc nhóm nào.
					continue;
				}

				// Giảm số hàng còn lại trong nhóm
				if (rowsRemainingInGroup > 0) {
					rowsRemainingInGroup--;
				}

				// Lấy SKU
				var skuInput = row.find(`td:nth-child(${skuColumnIndex}) input`);

				if (skuInput.length > 0 && skuInput.val()) {
					skuValue = skuInput.val().trim().toUpperCase();
				}

				// --- Logic ánh xạ ảnh ---
				if (skuValue && groupName && groupName.length > 0) {

					if (!mappingData[groupName]) {
						mappingData[groupName] = {
							skus: [],
							files: null,
					...
					// Thêm SKU vào nhóm đã được xác định bởi logic rowspan
					mappingData[groupName].skus.push(skuValue);

					// Dò SKU trong inputMap để tìm hình ảnh
					if (inputMap[skuValue] && inputMap[skuValue][0] && inputMap[skuValue][0].files.length > 0) {
						// Nếu tìm thấy hình, gán cho Tên Cấp 1 (Chỉ 1 lần cho cả nhóm)
						if (mappingData[groupName].files === null) {
							mappingData[groupName].files = inputMap[skuValue][0].files;
							mappingData[groupName].representativeSku = skuValue;
							boxLogging(`Tìm thấy ảnh cho nhóm ${groupName} bằng SKU: ${skuValue}`, [`${groupName}`], ["green"]);
						}
					}
				}
			}

			const groupsToProcess = Object.values(mappingData).filter(group => group.files !== null);

			if (groupsToProcess.length === 0) {
				boxLogging("Không tìm thấy Tên Phân loại Cấp 1 nào trên sàn có file ảnh tương ứng đã nạp.", ["Không tìm thấy nhóm có file ảnh."], ["orange"]);
				boxToast("Không tìm thấy nhóm SKU phù hợp!", "warning");
				return;
			}
			boxLogging(`Tìm thấy ${groupsToProcess.length} nhóm phân loại (Cấp 1) có ảnh cần thêm/sửa trên sàn.`, [`${groupsToProcess.length}`], ["blue"]);

			// **Bước 2: Xử lý ảnh (Giữ nguyên logic chống lỗi và delay)**
			var variantImageContainers = $(".prop-group-body > div:nth-child(2) > div:nth-child(2) > div .prop-option-list .next-form-item");

			if (variantImageContainers.length === 0) {
				boxLogging("Không tìm thấy các khối UI để tải ảnh biến thể.", [], ["red"]);
				boxToast("Không tìm thấy UI upload ảnh!", "error");
				return;
			}

			// --- Hàm deleteExistingImagesRecursive (Giữ nguyên logic và delay) ---
			async function deleteExistingImagesRecursive(containerElement) {
				try {
					...
					while (true) {
						...
						var imgItem = existingImages.eq(0);
						...
						if (deleteButton.length > 0) {
							const prevImageCount = existingImages.length;
							...
							if (!imageRemoved) {
								throw new Error("Không thể xác nhận ảnh đã được xóa sau khi click.");
							}
							retryCount = 0;
						} else {
							retryCount++;
							if (retryCount >= maxRetries) {
								throw new Error("Không tìm thấy nút xóa sau nhiều lần thử.");
							}
							await delay(250);
						}
					}
			...
			async function processVariantRecursive(index) {
				...
				// --- BỌC TRY/CATCH LỚN: BẢO ĐẢM KHÔNG DỪNG SCRIPT ---
				try {
					...
					if (nameInputForVariant.length === 0 || !nameInputForVariant.val()) {
						...
						const variantNameC1 = nameInputForVariant.val().trim();
						const matchedGroup = mappingData[variantNameC1];

						if (!matchedGroup || !matchedGroup.files) {
							boxLogging(`Không tìm thấy nhóm phân loại ${variantNameC1} có ảnh. Bỏ qua.`, [`${variantNameC1}`], ["orange"]);
						} else {
							const filesToUpload = Array.from(matchedGroup.files);
							const representativeSku = matchedGroup.representativeSku;

							// --- 1. XÓA ẢNH CŨ (BỌC TRY/CATCH ĐỂ BỎ QUA LỖI XÓA) ---
							try {
								await deleteExistingImagesRecursive(currentVariantContainer);
							} catch (e) {
								boxLogging(`[CẢNH BÁO] Lỗi khi xóa ảnh cũ cho ${variantNameC1}. Vẫn cố gắng gán ảnh mới.`, [], ["yellow"]);
							}

							// --- 2. GÁN VÀ TẢI ẢNH MỚI ---
							try {
								if (fileInputTarget.length > 0) {
									...
									await delay(700); // Đợi Lazada bắt đầu quá trình upload

									boxLogging(`Hình cho nhóm ${variantNameC1} (SKU: ${representativeSku}) đã được sửa`, [`${variantNameC1}`], ["lightgreen"]);
									...
									boxLogging(`Không thể sửa hình cho nhóm ${variantNameC1}: Không tìm thấy input file/drop zone.`, [`Không thể sửa hình`], ["red"]);
								}
							} catch (e) {
								boxLogging(`[LỖI NGHIÊM TRỌNG] Không thể gán/tải ảnh mới cho ${variantNameC1}.`, [], ["red"]);
							}
						}
					}

				} catch (error) {
					boxLogging(`[LỖI CHÍNH] Lỗi khi xử lý biến thể ${index}: ${error.message || error}. Tiếp tục biến thể tiếp theo.`, [], ["red", "bold"]);
				}
				// --- KẾT THÚC KHỐI TRY/CATCH LỚN ---

				// ĐẢM BẢO LUÔN GỌI ĐỆ QUY CHO BIẾN THỂ TIẾP THEO
    ...
    function chuanHoaSKULazada() {
      ...
      async function nextBox() {
        ...
        if (!skuInfo.sku.length == 0) {
          ...
          finalSku = await verifySKU({
            sku: "x0"
          }); // Chờ Promise được giải quyết, truyền object hợp lệ
        }
    ...
    async function layPhanLoaiLazada() {
      boxAlert("LẤY PHÂN LOẠI");
      ...
      for (let i = 0; i < tableRows.length; i++) {
        ...
        var price = row.find("td:nth-child(2) input");
      ...
      console.log(mappingData, variantImageContainers);

      let currentList = [];
      var zip = new JSZip();

      for (let i = 0; i < mappingData.length; i++) {
        var name = mappingData[i].name;
        // var detailRow = boxDetail.eq(i).find(".table-cell");
        var price = mappingData[i].price;
        let stockVal = mappingData[i].stock;
        var skuText = mappingData[i].sku;
        ...
        var img = variantImageContainers.eq(i).find("img");
        if (img.length) {
    ...
    function setEventSuaTenSKUShopee() {
      $(".tp-container.tp-content .layout-future #data").on("change", function() {
        var data = $(this).val();
        ...
        $.each(data, (index, value) => {
          ...
          var len = value[value.length - 1].length;
    ...
    function suaTenSKUShopee() {
      ...
      function nextData() {
        ...
        var current_data = data[indexData].split("\t");
        var sku = current_data[0];
        var name = current_data[1];
        var len = current_data[2];

        if (len > 20) {
          boxLogging("Số ký tự vượt quá giới hạn cho phép", ["Số ký tự vượt quá giới hạn cho phép"], ["crimson"]);
          indexData++;
          nextData();
          return;
        }
        ...
        function nextBoxDetail() {
          ...
          if (skuBox.val().includes(sku)) {
            ...
            if (last_name != name) {
              simulateClearReactInput(box.eq(indexBoxDetail).find("input"));
              simulateReactInput(box.eq(indexBoxDetail).find("input"), name);
              box.eq(indexBoxDetail).find("input").css("background", "lightgreen");
              boxLogging(`Đã sửa tên cho SKU ${sku} | ${last_name} thành ${name}`, [`${sku}`, last_name, name], ["green", "green", "green"]);
            } else {
              box.eq(indexBoxDetail).find("input").css("background", "orange");
              boxLogging(`Giữ nguyên tên ${last_name} cho SKU ${sku}`, [`${sku}`, last_name], ["orange", "orange"]);
            }
          }
    ...
    // Tính giá bán trước KM
    function tinhGiaTruocKM() {
      boxAlert("TÍNH GIÁ TRƯỚC KHUYẾN MÃI");
      var percent = $(".tp-container.tp-content .layout-future #percent").val()
      ...
      function nextData() {
        ...
        data_list.push(parseInt(data[indexData] * (1 + percent / 100)));
    ...
    function gopGiaBan() {
      ...
      function nextData() {
        ...
        data_list.push(gopGia(giaTruoc, giaSau).gia);
    ...
    function tachGiaBan() {
      ...
      function nextData() {
        ...
        var gia_tach = tachGia(data[indexData]);
    ...
    // Lấy phân loại TikTok Shop (Lấy dữ liệu cho TẤT CẢ biến thể)
    async function layPhanLoaiTiktok() {
      boxLogging("LẤY DỮ LIỆU PHÂN LOẠI TIKTOK SHOP", [], ["blue"]);
      boxToast("Đang xử lý dữ liệu phân loại TikTok...", "info");

      let currentList = [];
      var zip = new JSZip();
      let imageBlobs = [];
      let skuToImageMap = {};
      let imageBlobIndex = 0;

      // --- SELECTORS TIKTOK SHOP (Dựa trên cấu trúc thường gặp) ---
      // Khối chứa các phân loại Cấp 1 (Chứa Tên Cấp 1 và ảnh)
      var variantGroupContainers = $(".variant-attr-panel .variant-attr-value-list-item");
      // Bảng SKU (Chứa SKU, Giá, Tồn kho)
      var skuTableRows = $(".sku-table-container table tbody tr");

      if (variantGroupContainers.length === 0 || skuTableRows.length === 0) {
        boxLogging("Không tìm thấy dữ liệu phân loại hoặc bảng SKU. Vui lòng kiểm tra lại giao diện.", [], ["red"]);
        boxToast("Không tìm thấy dữ liệu phân loại!", "error");
        return;
      }
      boxLogging(`Tìm thấy ${variantGroupContainers.length} nhóm phân loại Cấp 1 và ${skuTableRows.length} dòng SKU.`, [], ["blue"]);

      // 1. Xử lý Tải Ảnh và Gán Tên Phân loại Cấp 1
      // Lặp qua các nhóm Cấp 1 để tải ảnh đại diện và lưu tên
      for (let gIndex = 0; gIndex < variantGroupContainers.length; gIndex++) {
        var groupContainer = variantGroupContainers.eq(gIndex);

        // Lấy Tên phân loại cấp 1
        var nameInput = groupContainer.find(".variant-attr-value-text input");
        const groupNameC1 = nameInput.val().trim() || `Phân loại ${gIndex + 1}`;

        // Lấy URL ảnh đại diện (giả định thẻ <img> nằm trong khối upload)
        var imgElement = groupContainer.find(".image-upload-wrapper img").eq(0);

        imageBlobs[imageBlobIndex] = null; // Mặc định không có ảnh

        if (imgElement.length && imgElement.attr("src")) {
          let url = imgElement.attr("src");

          try {
            // Tải ảnh (TikTok thường có CDN đơn giản hơn Shopee)
            var response = await fetch(url);
            var blob = await response.blob();

            // Chuyển sang PNG (tương tự logic cũ)
            var imageBitmap = await createImageBitmap(blob);
            var canvas = document.createElement("canvas");
            canvas.width = imageBitmap.width;
            canvas.height = imageBitmap.height;
            ...
            // LƯU BLOB ẢNH VÀO MẢNG
            imageBlobs[imageBlobIndex] = pngBlob;
            boxLogging(`Đã tải ảnh cho nhóm: ${groupNameC1}`, [], ["green"]);

          } catch (error) {
            boxLogging(`Lỗi tải/chuyển ảnh cho nhóm ${groupNameC1}: ${error.message}`, [], ["crimson"]);
          }
        }

        // Lưu Tên nhóm Cấp 1 và Chỉ số Blob vào Map tạm thời
        // Key: Tên Cấp 1 | Value: Chỉ số Blob
        skuToImageMap[groupNameC1] = imageBlobIndex;
        imageBlobIndex++; // Tăng chỉ số cho lần tải ảnh tiếp theo
      }

      // 2. Lặp qua Bảng SKU để thu thập dữ liệu và Gán ảnh
      for (let i = 0; i < skuTableRows.length; i++) {
        var row = skuTableRows.eq(i);
        var tableCells = row.find("td");

        if (tableCells.length < 5) {
          boxLogging(`Bỏ qua dòng SKU ${i+1}: Không đủ cột dữ liệu.`, [], ["yellow"]);
          continue;
        }

        // --- Giả định Cột TikTok ---
        // Cột 1: Tên Phân loại Cấp 1 (Text)
        // Cột 2: Tên Phân loại Cấp 2 (Text, có thể không có)
        // Cột 3: Giá (Input)
        // Cột 4: Tồn kho (Input)
        // Cột 5: SKU (Input)

        // Lấy Tên Phân loại Cấp 1 (Cột 1)
        const nameC1 = tableCells.eq(0).text().trim();
        // Lấy Tên Phân loại Cấp 2 (Cột 2)
        const nameC2 = tableCells.eq(1).text().trim();

        // Lấy Giá (Cột 3)
        var price = tableCells.eq(2).find("input").val() || "0";
        // Lấy Tồn kho (Cột 4)
        let stockVal = tableCells.eq(3).find("input").val() || "0";
        // Lấy SKU (Cột 5)
        var skuText = tableCells.eq(4).find("input").val() || `variant-${i}`;

        // --- Xử lý logic tồn kho (tương tự logic Shopee cũ của bạn) ---
        var getStock = $(".tp-container.tp-content .layout-future #getStock").prop("checked");
        if (!getStock) {
          var editStock = $(".tp-container.tp-content .layout-future #stock-edit").val();
          stockVal = editStock.length === 0 ? "0" : editStock;
        ...
        // --- Gán Chỉ số Ảnh ---
        // Dùng Tên Cấp 1 làm khóa tìm kiếm chỉ số Blob
        const blobIndex = skuToImageMap[nameC1];

        // Dữ liệu văn bản cần sao chép (tên cấp 1 + tên cấp 2 + SKU)
        var copyVariant = `${nameC1} | ${nameC2}\t${skuText}\t${price}\t${stockVal}`;
        currentList.push(copyVariant);

        // Lưu chỉ số Blob (đã tải ở Bước 1) vào Map mới, dùng SKU làm khóa
        skuToImageMap[skuText] = blobIndex;
      }

      // 3. Xử lý Tải Ảnh vào ZIP
      let imagesAddedCount = 0;
      for (let i = 0; i < currentList.length; i++) {
        const parts = currentList[i].split('\t');
        const sku = parts[1]; // Lấy SKU từ dữ liệu đã thu thập
        const blobIndex = skuToImageMap[sku]; // Lấy chỉ số Blob theo SKU
      ...
      // 4. Kết thúc: Sao chép và Nén ZIP
      navigator.clipboard.writeText(currentList.join("\n"));

      boxToast(`Đã sao chép ${currentList.length} phân loại`, "success");

      var downloadImage = $(".tp-container.tp-content .layout-future .layout-tab .switch-wrapper .switch #toggle-switch").prop("checked");
      ...
      if (downloadImage) {
        ...
        zip.generateAsync({
          ...
          saveAs(zipBlob, "Anh_Phan_Loai_TikTok_PNG.zip");
        });
      }
    }
