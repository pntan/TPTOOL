	...
	window.onload = function(){
		...
		function boxLogging(text, words = [], colors = []){
			...
			if (words.length === 0) {
				...
				log.html(`${data}\n(${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}) ${text}`);
			}
		...
		function flashSaleShopee(){
            ...
            $.each(container, (index, value) => {
                ...
                $.each(productBox, (index, value) => {
                    ...
                    var pos = nameList.findIndex(item => item.toLowerCase().trim().includes(productName.text().toLowerCase().trim()));
                    ...
                    if(pos != -1){
                        ...
                        if(parseInt(currentStock.text()) >= parseInt(count)){
                            ...
                            simulateClearing(campaignStock, 50, function(){
                                simulateTyping(campaingStock, (count));
                        ...
                        boxLogging(`Sản phẩm ${nameList[index]} không có trong danh sách`, [`${nameList[pos]}`], ["crimson"]);
                    }
                });
            });
